    ...
    <v-container>
      <v-row>
        ...
        > -->
          ...
          <v-card text class="d-flex flex-column align-center" v-if="page?.hero">
            <!-- <p class="font-20-800 pt-1">Welcome to JRMS</p> -->
            <p class="font-20-800 pa-2" style="text-transform: uppercase;">{{ page.hero.richText[1]?.children[0]?.text }}</p>
            <p class="font-34-800 custom-text pa-2">{{ page.hero.richText[3]?.children[0]?.text }}</p>
            <!-- <p>{{ page.hero.richText[2]?.children[0]?.text }}</p> -->
            <v-btn variant="outlined" class="font-20-800 pa-2 mb-4" @click="goToCreatePage" color="white">{{ page.hero.links[0]?.link.label }}</v-btn>
          </v-card>
            ...
            <v-row>
              <v-col v-for="block in page.layout" :key="block.id">
                ...
                <!-- <div v-if="block.media">
                  <img v-if="block.media.alt === 'img2'" :src="block.media.url" alt="Image description" />
                </div> -->
              </v-col>
            </v-row>
          </div>
